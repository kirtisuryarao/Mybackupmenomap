        ...
        <div className="grid grid-cols-7 gap-2">
          ...
          {monthDays.map((dayInfo, idx) => {
            ...
            return (
              <div
                key={idx}
                className={`aspect-square flex flex-col items-center justify-center rounded-lg p-1 transition-colors ${phaseColor} ${
          ...
          {(Object.entries(PHASE_LABELS) as Array<[CyclePhase, string]>).map(
            ([phase, label]) => (
              <div key={phase} className="flex items-center gap-2">
                <div className={`h-3 w-3 rounded-full ${PHASE_COLORS[phase].split(' ')[0]}`} />
